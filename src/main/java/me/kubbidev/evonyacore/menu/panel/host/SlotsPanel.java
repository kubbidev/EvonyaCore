...
public class SlotsPanel extends Menu {

    public SlotsPanel(PlayerMenuUtility playerMenuUtility, EvonyaPlugin plugin) {
    ...
    public void setMenuItems() {
        ...
        inventory.setItem(49, new Item(Material.REDSTONE_TORCH_ON)
                .setName("&8┃&6 Slots")
                .setLore(Collections.singletonList("  &8•&a " + gameInstance.getSlots()))
                .addEnchantments(Enchantment.DURABILITY, 1)
                .addItemFlags(ItemFlag.HIDE_ENCHANTS)
                .parseItem());
        inventory.setItem(47, new Head("&cEnlever 2 slots").setHeadType(HeadType.RED_MINUS).parseItem());
        inventory.setItem(48, new Head("&eEnlever 1 slot").setHeadType(HeadType.YELLOW_MINUS).parseItem());
        inventory.setItem(50, new Head("&eAjouter 1 slot").setHeadType(HeadType.YELLOW_PLUS).parseItem());
        inventory.setItem(51, new Head("&cAjouter 2 slots").setHeadType(HeadType.RED_PLUS).parseItem());
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        switch (material) {
            ...
            case REDSTONE_COMPARATOR:
                new StartingTimePanel(playerMenuUtility, plugin).open();
                return;
            case SLIME_BALL:
            ...
            case BEACON:
                ...
                return;
            case SKULL_ITEM:
                final int slots = gameInstance.getSlots();
                int finalSlots = slots;

                if (name.contains("Ajouter")) {
                    if (name.contains("1"))
                        finalSlots = (slots + 1);
                    else finalSlots = (slots + 2);
                }
                ...
                gameInstance.setSlots(finalSlots);
                player.sendMessage(EvonyaPlugin.PREFIX + "Slots &8»&c " + finalSlots + "&f Joueurs.");
                break;
        }
        super.open();
    }
